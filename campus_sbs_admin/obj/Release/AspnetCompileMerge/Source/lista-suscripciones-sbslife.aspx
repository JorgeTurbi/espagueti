<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="lista-suscripciones-sbslife.aspx.cs" Inherits="campus_sbs_admin.lista_suscripciones_sbslife" %>
...
<head runat="server">
    <title>SBS | Lista de suscripciones SBS Life</title>
...
<body>    
        ...
        <section class="padding-tb-40 padding-xs-tb-30">
            <div class="block-primary">
                <div class="row no-margin padding-nav">
                    <div class="col-sm-12">
                        <fieldset>
                            <legend id="txt_suscripciones" class="text-color-primary" runat="server"></legend>
                        </fieldset>
                    </div>
                    <div id="table_listado_suscripciones" class="col-sm-12" runat="server"></div>

                    <!-- Hidden y botones ocultos -->
                    <form id="Form1" accept-charset="utf-8" runat="server">
                        <asp:HiddenField ID="hidIdProducto" runat="server" />
                        <asp:HiddenField ID="hidIdUsuario" runat="server" />
                        <asp:HiddenField ID="hidFecha" runat="server" />
                        <asp:ImageButton ID="btnActivarSuscripcion" runat="server" ImageUrl="~/App_Themes/support/img/pixel.png" CssClass="hidden" OnClick="btnActivarSuscripcion_Click" />
                        <asp:ImageButton ID="btnBorrarSuscripcion" runat="server" ImageUrl="~/App_Themes/support/img/pixel.png" CssClass="hidden" OnClick="btnBorrarSuscripcion_Click" />
                        <asp:ImageButton ID="btnAllSuscripciones" runat="server" ImageUrl="~/App_Themes/support/img/pixel.png" CssClass="hidden" OnClick="btnAllSuscripciones_Click" />
                    </form>
   ...
   <!-- Scripts
    ...
    <script type="text/javascript">    
        ...
        function mostrarTodasSuscripciones() {
            $('#btnAllSuscripciones').click();
        }

        function eliminarSuscripcion(idProducto, idUsuario, fecha) {
            $('#hidIdProducto').val(idProducto);
            $('#hidIdUsuario').val(idUsuario);
            $('#hidFecha').val(fecha);

            $('#btnBorrarSuscripcion').click();
        }

        function activarSuscripcion(idProducto, idUsuario, fecha) {
            $('#hidIdProducto').val(idProducto);
            $('#hidIdUsuario').val(idUsuario);
            $('#hidFecha').val(fecha);

            $('#btnActivarSuscripcion').click();
        }
    </script>
</body>
</html>