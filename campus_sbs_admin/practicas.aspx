<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="practicas.aspx.cs" Inherits="campus_sbs_admin.practicas" %>
        ...
        <section class="padding-tb-40 padding-xs-tb-30">
            <div class="block-primary">
                <div class="row no-margin padding-nav">
                    <div class="col-sm-12">
                        <fieldset>
                            <legend id="txt_practicas" class="text-color-primary" runat="server"></legend>
                        </fieldset>
                    </div>
                    <div id="table_listado_practicas" class="col-sm-12" runat="server"></div>

                    <!-- Hidden y botones ocultos -->
                    <form id="Form1" accept-charset="utf-8" runat="server">
                        <asp:HiddenField ID="hidIdPractica" runat="server" />
                        <asp:HiddenField ID="hidMonth" runat="server" />
                        <asp:ImageButton ID="btnBorrarPractica" runat="server" ImageUrl="~/App_Themes/support/img/pixel.png" OnClick="btnBorrarPractica_Click" CssClass="hidden" />
                        <asp:ImageButton ID="btnCrearExcel" runat="server" ImageUrl="~/App_Themes/support/img/pixel.png" OnClick="btnCrearExcel_Click" CssClass="hidden" />
                        <asp:ImageButton ID="btnTodasPracticas" runat="server" ImageUrl="~/App_Themes/support/img/pixel.png" CssClass="hidden" OnClick="btnTodasPracticas_Click" />
                        <asp:ImageButton ID="btnFinPracticas" runat="server" ImageUrl="~/App_Themes/support/img/pixel.png" CssClass="hidden" OnClick="btnFinPracticas_Click" />
                    </form>
    ...
    <script type="text/javascript">    
        $(document).ready(function () {
            $('#tabla_Practicas').DataTable({
                ...
                lengthMenu: [[-1, 20, 50], ["Todos", 20, 50]],
                "columnDefs": [
                  {
                      "targets": [0],
                      "class": "text-center",
        ...
        function eliminarPractica(id) {
            var hidId = document.getElementById('<%=hidIdPractica.ClientID %>');
            hidId.value = id;

            var boton = document.getElementById('<%=btnBorrarPractica.ClientID %>');
            boton.click();
        }
        ...
        function all_practices() {
            var boton_all = document.getElementById('<%=btnTodasPracticas.ClientID %>');
            boton_all.click();
        }

        function end_practices() {
            /// 1.- Sacar el mes
            var month = $('#ddlMonth').val();
            $('#hidMonth').val(month);

            /// 2.- Pulsar el botón
            var boton_end = document.getElementById('<%=btnFinPracticas.ClientID %>');
            boton_end.click();
        }
    </script>
</body>
